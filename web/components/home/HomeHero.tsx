...
export const HomeHero = ({
  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:bg-slate-800 dark:border-slate-700">
      <div className="flex flex-col gap-6 p-6 sm:p-8 lg:flex-row lg:items-center lg:justify-between">
        <div className="max-w-2xl">
          ...
          <h1 className="mt-4 text-2xl font-semibold tracking-tight text-slate-900 text-balance sm:text-3xl dark:text-white">
            Bienvenido a Manus POS
          </h1>
          <p className="mt-1 text-base font-medium text-slate-700 dark:text-slate-200">
            {businessName}
          </p>
          <p className="mt-3 text-sm leading-relaxed text-slate-600 text-pretty dark:text-slate-300">
            Hola, <span className="font-semibold text-slate-900 dark:text-white">{userName}</span>.
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            ...
            <Link
              href={`/${tenant}/finance`}
              className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-900 shadow-sm transition hover:-translate-y-0.5 hover:bg-slate-50 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300 dark:bg-slate-800 dark:border-slate-700 dark:text-white"
            >
        ...
        <div className="rounded-2xl border border-slate-100 bg-slate-50 p-5 lg:w-64">
          ...
          <div className="mt-3 space-y-2 text-sm">
            ...
            <p className="text-slate-600 dark:text-slate-300">Módulos principales disponibles</p>
          </div>
        </div>
      </div>
    </section>
