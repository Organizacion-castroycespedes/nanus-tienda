...
- JWT identifies the user, tenant and roles. Database assignments determine branch scope. `REPORTS_ALLOW_MOCK_AUTH` is valid only with `NODE_ENV=development` or `NODE_ENV=test`; QA and production always require a valid Bearer JWT, even if the variable is set to `true`.
