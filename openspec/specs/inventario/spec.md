...
Supuesto heredado: el ledger actual `stock_movements` se mantiene como base de trazabilidad.

Preguntas abiertas heredadas:

- La fecha operativa para vencimientos puede requerir definicion entre fecha del servidor, fecha local de sucursal o fecha de caja.
- La rotacion puede requerir definicion por unidades, valor vendido, margen, dias sin movimiento o combinacion.

Riesgos heredados:

- Si FEFO no se ejecuta dentro de la misma transaccion de venta, dos cajas podrian consumir el mismo lote.
- Si saldos por lote y `stock_movements` divergen, los reportes y POS mostraran datos distintos.
## Requirements
