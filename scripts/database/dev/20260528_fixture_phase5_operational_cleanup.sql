...
-- Limpia unicamente datos controlados de Fase 5 integral operativa.

\set ON_ERROR_STOP on

DO $$
DECLARE
  c_tenant_id CONSTANT UUID := '95000000-0000-0000-0000-000000000001';
...
BEGIN
  ...
  DELETE FROM product_barcodes WHERE tenant_id = c_tenant_id;
  IF to_regclass('purchase_status_history') IS NOT NULL THEN
    DELETE FROM purchase_status_history WHERE tenant_id = c_tenant_id;
  END IF;
  DELETE FROM purchase_items
  WHERE purchase_id IN (SELECT id FROM purchases WHERE tenant_id = c_tenant_id);
  DELETE FROM purchases WHERE tenant_id = c_tenant_id;
  IF to_regclass('product_price_history') IS NOT NULL THEN
    DELETE FROM product_price_history WHERE tenant_id = c_tenant_id;
  END IF;
  DELETE FROM products WHERE tenant_id = c_tenant_id;
  DELETE FROM inventory_locations WHERE tenant_id = c_tenant_id;
  ...
  DELETE FROM suppliers WHERE tenant_id = c_tenant_id;
  DELETE FROM user_roles WHERE tenant_id = c_tenant_id;
  ...
  DELETE FROM auditoria_eventos WHERE tenant_id = c_tenant_id;
  DELETE FROM tenants WHERE id = c_tenant_id;
END $$;

WITH fixture AS (
  SELECT '95000000-0000-0000-0000-000000000001'::UUID AS tenant_id
)
SELECT
  (
    ...
    + (SELECT COUNT(*) FROM purchases WHERE tenant_id = fixture.tenant_id)
