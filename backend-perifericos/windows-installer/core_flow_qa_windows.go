...
window.manusInstaller.onState=function(snapshot){
  ...
  if(snapshot.phase==='FAILED_SAFE'){var rollback=(snapshot.steps||[]).find(function(step){return step.id==='ROLLBACK';});if(!rollback){var terminalMessage=section.querySelector('[data-core-terminal-message]');if(terminalMessage)terminalMessage.textContent=snapshot.error||'No pudimos completar la instalación.';}var technical=section.querySelector('[data-core-technical-error]');if(!technical&&snapshot.technicalError){technical=document.createElement('pre');technical.dataset.coreTechnicalError='true';technical.className='core-terminal-message';technical.textContent='Detalle técnico: '+snapshot.technicalError;section.querySelector('.heading').append(technical);}document.body.dataset.coreError=snapshot.error||'FAILED_SAFE';}
