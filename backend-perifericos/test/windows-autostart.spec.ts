...
test("windows autostart runner targets portable node and health gate", () => {
  ...
  assert.ok(packageScript.includes('if not defined PERIPHERALS_VERSION set "PERIPHERALS_VERSION='));
  ...
  assert.ok(packageScript.includes('$configPath = $env:PERIPHERALS_CONFIG_PATH'));
  assert.ok(packageScript.includes('PERIPHERALS_CONFIG_PATH = $configPath'));
  assert.ok(packageScript.includes('PERIPHERALS_VERSION = $defaultVersion'));
